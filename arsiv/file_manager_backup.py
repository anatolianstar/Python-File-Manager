...
Değiştirilme: {modified}"""
    ...
    def _prepare_scan(self):
        ...
        total_files = 0
        processed_files = 0
        all_scanned_files = []  # Tüm taranan dosyalar
        ...
        # Hedef klasör dosyalarını da dahil et
        all_files_for_duplicate_check = all_scanned_files.copy()
        
        # Hedef klasör mevcut mu ve duplikat kontrol yapılsın mı?
        if self.target_path and os.path.exists(self.target_path):
            target_files = []
            ...
            if self.debug_mode:
                print(f"🔍 Hedef klasör dosyaları dahil ediliyor: {len(target_files)} dosya")
            
            # Hedef klasör dosyalarını listeye ekle
            all_files_for_duplicate_check.extend(target_files)
        
        # Duplikat tespiti yap (kaynak + hedef dosyalar)
        self.find_duplicates_advanced(all_files_for_duplicate_check)
        
        # Duplikat tree'sini doldur
        self.populate_duplicate_tree()
        
        # İstatistikleri hesapla
        total_duplicates = sum(len(files) for files in self.duplicate_groups.values())
        unique_count = len(self.unique_files)
        
        scan_mode = "tüm alt klasörler dahil" if self.scan_subfolders.get() else "sadece ana klasör"
        self.status_var.set(f"Tarama tamamlandı. {total_files} dosya | {unique_count} benzersiz | {total_duplicates} duplikat ({scan_mode})")
    ...
    def _scan_source_files(self):
        ...
        # Organizasyon önizleme tree'sini güncelle
        for category, stats in extension_stats.items():
            category, cat_info = self.get_file_category_info(category)
            folder_name = cat_info['folder']
            
            # Ana klasör ekle
            parent_item = self.preview_tree.insert('', 'end', text=folder_name, 
                                                 values=(f"{stats} dosya"))
    ...
    def _detect_duplicates(self):
        ...
        if self.debug_mode:
            print(f"🔍 Duplikat kontrol başlıyor: {len(all_scanned_files)} dosya")
        ...
        for file_path in all_scanned_files:
            try:
                ...
                if processed % 100 == 0 and self.debug_mode:
                    print(f"İşlenen: {processed}/{len(all_scanned_files)}")
